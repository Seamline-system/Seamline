/* =============================================
   SEAMLINE Quotation System — app.js
...
let salesSortMode      = 'revenue';
let historySearch      = '';
let historyFilterStatus = 'all';
let paymentFilter      = 'all'; // 'revenue' | 'profit' | 'orders'

// ── Embedded SEAMLINE Logo ─────────────────────
...
function updateSummary() {
  ...
  const pct    = totalRev > 0 ? ((profit/totalRev)*100).toFixed(1) : '—';
  ...
  document.getElementById('profitMarginText').textContent =
    `SP (Rs.${Math.abs(totalRev).toFixed(2)}) − UP (Rs.${Math.abs(totalCost).toFixed(2)}) = ${profit>=0?'+':''}Rs.${profit.toFixed(2)}`;
  document.getElementById('marginPct').textContent = totalRev > 0 ? pct + '%' : '—';
}
...
window.saveQuote = async function() {
  ...
  try {
    if (editingDocId) {
      ...
      showToast('✓ Quote saved to cloud!');
    }
    clearForm();
    updateCount();
  } catch(e) {
...
function renderHistory() {
  ...
  const countEl   = document.getElementById('historyResultsCount');
  ...
  if (historyFilterStatus !== 'all') {
    filtered = filtered.filter(q => (q.status||'pending') === historyFilterStatus);
  }

  // Update count
  if (countEl) {
    countEl.textContent = filtered.length === quotes.length
  ...
  if (filtered.length === 0) {
    container.innerHTML = quotes.length === 0
      ? `<div class="empty-state"><div class="icon">🗂</div><h3>No quotes yet</h3><p>Create your first quotation to see it here.</p></div>`
      : `<div class="empty-state"><div class="icon">🔍</div><h3>No results</h3><p>Try a different search or filter.</p></div>`;
    return;
  }
  ...
  container.innerHTML = filtered.map(q => {
    ...
    const clientHl    = searchTerm ? highlight(q.client||'', searchTerm) : (q.client||'');
    const refHl       = searchTerm ? highlight(q.ref||'', searchTerm)    : (q.ref||'');
    return `
    <div class="quote-card">
      <div class="quote-card-header">
        <div>
          <div class="quote-num">${refHl} <span class="status-badge ${status}">${statusLabel}</span></div>
          <div class="quote-date">${clientHl} · ${q.contact||'No contact'} · ${formatDate(q.date)}
            <span style="margin-left:8px;color:#777;font-size:11px;">by ${q.createdBy||'unknown'}</span>
          </div>
        </div>
        <span class="quote-badge ${q.profit>=0?'badge-profit':'badge-loss'}">${q.profit>=0?'▲ Profit':'▼ Loss'}</span>
      </div>
      ...
      <div class="quote-card-actions">
        <button class="mini-btn" onclick="editQuote('${q._docId}')">✏️ Edit</button>
        <button class="mini-btn" onclick="duplicateQuote('${q._docId}')">📋 Duplicate</button>
        <button class="mini-btn" onclick="pdfFromHistory('${q._docId}')">📄 PDF</button>
        <button class="mini-btn del" onclick="deleteQuote('${q._docId}')">🗑 Delete</button>
...
window.editQuote = function(docId) {
  ...
  showToast('✏️ Editing quote ' + q.ref);
};
...
window.handleDesignUpload = function(event) {
  ...
  reader.onload = e => {
    ...
    document.getElementById('removeDesignBtn').style.display = 'inline-block';
    showToast('✓ Design image uploaded!');
  };
...
window.generatePDF = function(data) {
  ...
  d.items.forEach(item => {
    ...
    const clipped=doc.splitTextToSize(item.desc||'—',colDescR-colDescL)[0];
...
// ── Update Quote Status from History ──────────
window.updateQuoteStatus = async function(docId, newStatus) {
  try {
    await updateDoc(doc(db, 'quotes', docId), { status: newStatus });
    const label = newStatus === 'approved' ? '✓ Approved' : newStatus === 'rejected' ? '✕ Rejected' : '⏳ Pending';
...
function getApprovedByPeriod() {
  ...
  return quotes.filter(q => {
    ...
    if (period === 'custom') {
      ...
      if (fromVal) { const from = new Date(fromVal); if (d < from) return false; }
      if (toVal)   { const to   = new Date(toVal);   to.setHours(23,59,59); if (d > to) return false; }
      return true;
    }
    return true;
  });
...
window.onPeriodChange = function() {
  ...
  if (range) range.style.display = val === 'custom' ? 'flex' : 'none';
  renderSalesTracker();
};
        ...
        if (container) {
          container.innerHTML = `
            <div style="padding:32px;border:2px solid var(--danger);background:#fff8f8;color:var(--danger);">
              ...
              Update your Firestore rules in Firebase Console → Firestore → Rules:<br><br>
...
// ── Tab switch — hook into existing switchTab ──
...
// ── Kick off clients listener when auth is ready
...
// ── Start a new quote pre-filled for a client ──
window.startQuoteForClient = function(docId) {
  ...
  showToast(`✦ New quote for ${c.name}`);
};

// ═══════════════════════════════════════════════
// CLIENT AUTOCOMPLETE — New Quote Form
// ═══════════════════════════════════════════════
...
window.onClientNameInput = function(val) {
  ...
  if (matches.length) {
    ...
    html += `<div class="cd-no-match">No client named "<strong>${val}</strong>" — just keep typing to use as new client.</div>`;
  }
...
window.selectClientFromDropdown = function(docId) {
  ...
  // Fill contact — prefer phone, then email, then contact person name
...
window.drawerUpdateStatus = async function(quoteDocId, newStatus, clientDocId) {
  // Optimistic update local array first so openClientProfile reads fresh data
  const localQ = quotes.find(q => q._docId === quoteDocId);
  const prevStatus = localQ?.status;
  if (localQ) localQ.status = newStatus;
  try {
...
window.drawerDeleteQuote = async function(quoteDocId, clientDocId) {
  ...
  // Remove from local array immediately so stats refresh instantly
  const deletedIdx = quotes.findIndex(q => q._docId === quoteDocId);
  const deletedQ   = deletedIdx > -1 ? quotes.splice(deletedIdx, 1)[0] : null;
  try {
...
// ═══════════════════════════════════════════════
// BATCH 2 — HISTORY SEARCH, CSV, PAYMENTS
// ═══════════════════════════════════════════════
...
window.setHistoryFilter = function(status, btn) {
...
// ── Duplicate Quote ────────────────────────────
window.duplicateQuote = function(docId) {
  const q = quotes.find(q => q._docId === docId);
  if (!q) return;
  editingDocId = null; editingId = null;
  document.getElementById('clientName').value    = q.client;
  document.getElementById('clientContact').value = q.contact || '';
  document.getElementById('quoteNotes').value    = q.notes   || '';
  document.getElementById('quoteDate').value     = new Date().toISOString().split('T')[0];
  ...
  document.getElementById('panel-new').classList.add('active');
  document.querySelector('.tab-btn').classList.add('active');
  showToast('📋 Duplicated — review and save as new quote');
};

// ── CSV Export ─────────────────────────────────
window.exportCSV = function() {
  const rows = [['Ref','Client','Contact','Date','Status','Total Cost','Revenue','Profit','Margin %','Items','Created By']];
  quotes.forEach(q => {
    rows.push([
      q.ref||'', q.client||'', q.contact||'', q.date||'',
      q.status||'pending',
      (q.totalCost||0).toFixed(2),
      (q.totalRev||0).toFixed(2),
  ...
  a.download = `seamline-quotes-${new Date().toISOString().split('T')[0]}.csv`;
  a.click();
...
window.exportPaymentsCSV = function() {
  const approved = quotes.filter(q => (q.status||'pending') === 'approved');
  const rows = [['Ref','Client','Contact','Date','Revenue','Payment Status','Amount Paid','Balance']];
  approved.forEach(q => {
    const ps  = q.paymentStatus || 'unpaid';
    const paid = q.amountPaid || 0;
    const bal  = (q.totalRev||0) - paid;
    rows.push([q.ref||'', q.client||'', q.contact||'', q.date||'',
      (q.totalRev||0).toFixed(2), ps, paid.toFixed(2), bal.toFixed(2)]);
  });
  const csv  = rows.map(r => r.map(v => `"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n');
  const blob = new Blob([csv], { type: 'text/csv' });
  const a    = document.createElement('a');
  a.href     = URL.createObjectURL(blob);
  a.download = `seamline-payments-${new Date().toISOString().split('T')[0]}.csv`;
  a.click();
  showToast('✓ Payments CSV exported!');
};

// ── Payments Tracker ───────────────────────────
window.setPaymentFilter = function(filter, btn) {
  paymentFilter = filter;
  document.querySelectorAll('#panel-payments .filter-pill').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  renderPayments();
};
...
window.renderPayments = function() {
  ...
  // KPIs
  const totalRev     = approved.reduce((s,q) => s + (q.totalRev||0), 0);
  const totalPaid    = approved.reduce((s,q) => s + (q.amountPaid||0), 0);
  const totalUnpaid  = totalRev - totalPaid;
  const paidCount    = approved.filter(q => (q.paymentStatus||'unpaid') === 'paid').length;
  const partialCount = approved.filter(q => (q.paymentStatus||'unpaid') === 'partial').length;
  const unpaidCount  = approved.filter(q => (q.paymentStatus||'unpaid') === 'unpaid').length;

  const kpiEl = document.getElementById('paymentsKpi');
  if (kpiEl) {
    kpiEl.innerHTML = `
      <div class="pay-kpi-card"><div class="pay-kpi-label">Total Invoiced</div><div class="pay-kpi-val">Rs.${totalRev.toLocaleString('en-US',{minimumFractionDigits:2})}</div></div>
      <div class="pay-kpi-card"><div class="pay-kpi-label">Total Collected</div><div class="pay-kpi-val" style="color:var(--success)">Rs.${totalPaid.toLocaleString('en-US',{minimumFractionDigits:2})}</div></div>
      <div class="pay-kpi-card"><div class="pay-kpi-label">Outstanding</div><div class="pay-kpi-val" style="color:var(--danger)">Rs.${totalUnpaid.toLocaleString('en-US',{minimumFractionDigits:2})}</div></div>
      <div class="pay-kpi-card"><div class="pay-kpi-label">Paid</div><div class="pay-kpi-val">${paidCount}</div></div>
      <div class="pay-kpi-card"><div class="pay-kpi-label">Partial</div><div class="pay-kpi-val">${partialCount}</div></div>
      <div class="pay-kpi-card"><div class="pay-kpi-label">Unpaid</div><div class="pay-kpi-val" style="color:var(--danger)">${unpaidCount}</div></div>
    `;
  }
  ...
  if (paymentFilter !== 'all') {
    filtered = filtered.filter(q => (q.paymentStatus||'unpaid') === paymentFilter);
  }
  ...
  if (!filtered.length) {
    container.innerHTML = `<div class="empty-state"><div class="icon">💰</div><h3>No ${paymentFilter === 'all' ? 'approved' : paymentFilter} quotes</h3><p>${paymentFilter === 'all' ? 'Approve quotes to track payments here.' : 'None in this category.'}</p></div>`;
    return;
  }
  ...
  container.innerHTML = filtered.map(q => {
    const ps      = q.paymentStatus || 'unpaid';
    const paid    = q.amountPaid    || 0;
    const balance = (q.totalRev||0) - paid;
    const pct     = q.totalRev > 0 ? Math.min(100, Math.round((paid / q.totalRev) * 100)) : 0;
    const psLabel = ps === 'paid' ? '✓ Paid' : ps === 'partial' ? '◑ Partial' : '○ Unpaid';
    const psClass = ps === 'paid' ? 'ps-paid' : ps === 'partial' ? 'ps-partial' : 'ps-unpaid';
    return `
      <div class="payment-card">
        <div class="payment-card-top">
          <div>
            <div class="payment-ref">${q.ref} <span class="ps-badge ${psClass}">${psLabel}</span></div>
            <div class="payment-client">${q.client} · ${formatDate(q.date)}</div>
          </div>
          <div class="payment-amounts">
            <div class="pa-item"><span class="pa-label">Invoiced</span><span class="pa-val">Rs.${(q.totalRev||0).toLocaleString('en-US',{minimumFractionDigits:2})}</span></div>
            <div class="pa-item"><span class="pa-label">Paid</span><span class="pa-val" style="color:var(--success)">Rs.${paid.toLocaleString('en-US',{minimumFractionDigits:2})}</span></div>
            <div class="pa-item"><span class="pa-label">Balance</span><span class="pa-val" style="color:${balance>0?'var(--danger)':'var(--success)'}">Rs.${balance.toLocaleString('en-US',{minimumFractionDigits:2})}</span></div>
          </div>
        </div>
        <div class="payment-progress-wrap">
          <div class="payment-progress-track">
            <div class="payment-progress-fill ${psClass}" style="width:${pct}%"></div>
          </div>
          <span class="payment-pct">${pct}% collected</span>
        </div>
        <div class="payment-actions">
          <select class="payment-status-select" onchange="updatePaymentStatus('${q._docId}', this.value)">
          ...
          <div class="payment-amount-input-wrap">
            <span class="pa-currency">Rs.</span>
            <input type="number" class="payment-amount-input" value="${paid||''}" placeholder="Amount paid"
              min="0" step="0.01" onchange="updateAmountPaid('${q._docId}', this.value)">
          </div>
          <button class="mini-btn" onclick="pdfFromHistory('${q._docId}')">📄 PDF</button>
        </div>
      </div>`;
...
window.updatePaymentStatus = async function(docId, status) {
  // Update local array instantly — no waiting for Firestore round-trip
  const q = quotes.find(q => q._docId === docId);
  if (q) { q.paymentStatus = status; }
  renderPayments();
  try {
    await updateDoc(doc(db, 'quotes', docId), { paymentStatus: status });
    showToast(`Payment marked: ${status}`);
  } catch(e) {
    // Revert local on failure
    if (q) { q.paymentStatus = q._prevPaymentStatus || 'unpaid'; }
    renderPayments();
    showToast('Error updating payment');
  }
};

window.updateAmountPaid = async function(docId, amount) {
  const val = parseFloat(amount) || 0;
  const q   = quotes.find(q => q._docId === docId);
  if (!q) return;
  let status = 'unpaid';
  if (val > 0 && val >= (q.totalRev||0)) status = 'paid';
  else if (val > 0) status = 'partial';
  // Optimistic update — show change instantly
  const prevAmt    = q.amountPaid;
  const prevStatus = q.paymentStatus;
  q.amountPaid    = val;
  q.paymentStatus = status;
  renderPayments();
  try {
    await updateDoc(doc(db, 'quotes', docId), { amountPaid: val, paymentStatus: status });
    showToast('✓ Amount updated');
  } catch(e) {
    // Revert local on failure
    q.amountPaid    = prevAmt;
    q.paymentStatus = prevStatus;
    renderPayments();
    showToast('Error updating amount');
  }
};

// ── Search highlight helper (if not already defined) ──
if (typeof highlight === 'undefined') {
  window.highlight = function(text, query) {
    if (!query) return text;
